...
export default function FieldCreateMap({ isNew }: Props) {
  const router = useRouter();

  const apiKey = process.env.NEXT_PUBLIC_MAPS_API_KEY as string | undefined;
  const { isLoaded } = useJsApiLoader({
    id: "ripen-google-maps",
  ...
  const handleSave = () => {
    ...
    if (!country.trim()) {
      alert("Country is required (auto-filled from map search)");
      return;
    }
    ...
    const ring: [number, number][] = latestPath.map((p) => [p.lng, p.lat]) as [
      number,
      number
    ][];
    if (ring.length) {
    ...
    const farm = {
      ...
      thumbnailUrl:
        "https://images.unsplash.com/photo-1615676809288-f8cd02d19b5f?q=80&w=800&auto=format&fit=crop",
  ...
  const centroidOf = useCallback(
    (points: { lat: number; lng: number }[]): { lat: number; lng: number } => {
  ...
  const useMyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition((pos) => {
      ...
      if (
        isLoaded &&
        (window as unknown as { google?: typeof google }).google?.maps?.Geocoder
      ) {
        const geocoder = new google.maps.Geocoder();
        geocoder.geocode(
    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold">
            {isNew ? "Link New Farm" : "Edit Farm"}
          </h1>
          <p className="text-sm text-gray-500">
            Draw a single polygon to outline the farm.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3">
          {!apiKey ? (
            <div className="h-[480px] rounded-xl border flex items-center justify-center bg-white">
            ...
            <div className="bg-white rounded-xl border p-3 space-y-3 text-gray-900">
              <StandaloneSearchBox
                onLoad={(ref) => {
                  (
                    searchRef as unknown as {
                      current: google.maps.places.SearchBox | null;
                    }
                  ).current = ref;
                }}
              ...
              >
                <input
                  className="w-full border rounded-md px-3 py-2 text-gray-900 placeholder:text-gray-400 bg-white"
